...
export const GOAL_MOBILE_EFFECTS: PerfGoal = {
  ...
  checks: [
    ...
    todo('usePerfTier wired into HomeClient for below-fold sections',
      "HomeClient.tsx: import usePerfTier. On tier:low, skip VisualEffectsLayer entirely. Removes GSAP-driven background animations from low-end devices. Implementation: const { tier } = usePerfTier(); return tier === 'low' ? null : <VisualEffectsLayer />."),
  ],
}
...
export const GOAL_PRERENDER: PerfGoal = {
  ...
  checks: [
    ...
    todo('OG image and PWA icons served from dist/ with correct cache headers',
      'Verify that _headers (Cloudflare Pages) sets Cache-Control: public, max-age=31536000, immutable on /icon-*.png and /og-image.*. This prevents repeat network requests on return visits.'),
  ],
}
...
export const GOAL_SELF_HOST_FONTS: PerfGoal = {
  ...
  checks: [
    ...
    todo('Verify zero font CDN requests via dist HTML audit',
      "Check dist/en/index.html for fonts.googleapis.com or fonts.gstatic.com references. Should find 0. Ideally add to CI: grep -r 'fonts.googleapis' dist/ should exit with no matches."),
  ],
}
...
export const GOAL_IMAGES: PerfGoal = {
  ...
  dependsOn:  [],
  baseline:   'PWA icons are PNG. OG image is unverified. <img> tags in admin panels.',
  target:     'All public <img> tags have explicit width/height; admin images are admin-only',
  ...
  checks: [
    ...
    todo('PWA icons audited and WebP variants added',
      "Check public/icon-192x192.png, icon-512x512.png exist and are correct size. Add WebP variants via ImageMagick: cwebp -q 90 icon-512x512.png -o icon-512x512.webp. Update manifest.ts to include WebP purpose:'maskable any' alongside PNG."),
    todo('next/image configured for static export CDN',
      "next.config.ts: add images: { loader: 'custom', loaderFile: './src/lib/image-loader.ts' } pointing to Cloudflare Images or Cloudflare CDN. This enables proper srcset + WebP conversion for user-uploaded cover images in ContentEditorPanel. Fallback: images.unoptimized: true for Phase 4, replace with CDN loader in Phase 5."),
    todo('OG image dimensions verified (1200×630)',
      "Check that the OG image in the <meta og:image> tag is exactly 1200×630 px and ≤ 200 KB. The generate:og script should produce a correctly-sized file. Add CI check: node scripts/validate-og-image.mjs."),
  ],
}
...
export const GOAL_INP: PerfGoal = {
  ...
  checks: [
    ...
    todo('Long task breakdown chart in PerformanceTab',
      'Long tasks (>50ms) are collected via observeLongTasks(). Add a breakdown chart showing top long tasks that drive high INP: task name, duration, attribution (script URL). Helps diagnose which interaction causes worst INP.'),
  ],
}
...
export const GOAL_CI_BUDGETS: PerfGoal = {
  ...
  checks: [
    ...
    todo('Mobile Lighthouse run added to CI',
      "Add a second lhci job with mobile preset: screenEmulation.mobile:true, throttlingMethod:'simulate', mobileFormFactor. Track mobile scores separately. Mobile LCP target: < 4000ms. Mobile Performance target: ≥ 65."),
  ],
}
