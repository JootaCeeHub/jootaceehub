...
export const AdminStateSchema = z.object({
  ...
  studioConfig: z.object({
    ...
    panelOverrides: z.array(z.object({
      id: z.string(), visible: z.boolean(), order: z.number(),
      customLabel: z.string().optional(), customDesc: z.string().optional(),
      cardStyle:     z.enum(['filled', 'glass', 'outlined', 'flat']).optional(),
      accentOverride: z.string().optional(),
      widthOverride:  z.enum(['auto', 'sm', 'md', 'lg', 'full']).optional(),
    })).optional(),
    panelCardStyle:           z.enum(['filled', 'glass', 'outlined', 'flat']).optional(),
    panelCardBgOpacity:       z.number().min(0).max(100).optional(),
    panelCardBorderIntensity: z.number().min(0).max(100).optional(),
    panelCardGlow:            z.boolean().optional(),
    contentSectionOverrides: z.array(z.object({
      id:          z.string(),
